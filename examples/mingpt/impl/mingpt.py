...
from examples.mingpt.impl import utils
...
class CausalSelfAttention(nn.Module):
  ...
  def __init__(self, config, device):
    ...
    self.register_buffer(
        'bias',
        torch.tril(
            torch.ones(config.block_size, config.block_size, device=device)
...
class Block(nn.Module):
  ...
  def __init__(self, config, device):
    super().__init__()
    assert config.n_embd is not None
    self.ln_1 = nn.LayerNorm(config.n_embd, device=device)
    self.attn = CausalSelfAttention(config, device=device)
    self.ln_2 = nn.LayerNorm(config.n_embd, device=device)
    self.mlp = nn.ModuleDict(
        dict(
            c_fc=nn.Linear(config.n_embd, 4 * config.n_embd, device=device),
            c_proj=nn.Linear(4 * config.n_embd, config.n_embd, device=device),
            act=NewGELU(),
...
class GPT(nn.Module):
  ...
  def configure_optimizers(self, train_config):
    ...
    We are separating out all parameters of the model into two buckets: those
    ...
    decay = set()
    no_decay = set()
    decay_weight_modules = (torch.nn.Linear,)
    no_decay_weight_modules = (torch.nn.LayerNorm, torch.nn.Embedding)
    for mn, m in self.named_modules():
      for pn, p in m.named_parameters():
        ...
        if pn.endswith('bias'):
          ...
          no_decay.add(fpn)
        elif pn.endswith('weight') and isinstance(m, decay_weight_modules):
          # weights of allowed modules will be weight decayed
          decay.add(fpn)
        elif pn.endswith('weight') and isinstance(m, no_decay_weight_modules):
          # weights of no_decay modules will NOT be weight decayed
          no_decay.add(fpn)
  ...
  def forward(self, idx, targets=None):
    ...
    for block in self.transformer.h:
      x = block(x)
    # TODO(mvoz): Tiny delta in numerical accuracy here.
