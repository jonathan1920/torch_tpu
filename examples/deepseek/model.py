...
This is lightly adapted from the original model in
https://github.com/deepseek-ai/DeepSeek-V3/blob/main/inference/model.py

The only non-style change is setting the kernels to None for now. All
operations have a pure PyTorch fallback mode, which is even better
for testing op and layer coverage of torch_tpu.

Every call to a collective (dist.*) is guarded by a check for world_size > 1,
so for op / layer testing, we can set world_size = 1.
"""
...
# These were cuda kernels and including them would require
# adding a module that has multiple dependencies.
def _callable(*_, **__):  # pylint: disable=unused-argument
  raise NotImplementedError("Not implemented")


act_quant = weight_dequant = fp8_gemm = _callable
...
class ParallelEmbedding(nn.Module):
  ...
  def __init__(self, vocab_size: int, dim: int):
    """Initializes the parallel embedding layer.
  ...
  def forward(self, x: torch.Tensor) -> torch.Tensor:  # pylint: disable=redefined-outer-name
...
def linear(
    x: torch.Tensor, weight: torch.Tensor, bias: Optional[torch.Tensor] = None  # pylint: disable=redefined-outer-name
) -> torch.Tensor:
  ...
  if weight.element_size() > 1:
    ...
    assert hasattr(weight, "scale")  # Workaround for pytype.
...
class RowParallelLinear(Linear):
  ...
  def __init__(
    ...
    """Initializes the row parallel linear layer.
...
class RMSNorm(nn.Module):
  ...
  def forward(self, x: torch.Tensor):  # pylint: disable=redefined-outer-name
...
def precompute_freqs_cis(args: ModelArgs) -> torch.Tensor:  # pylint: disable=redefined-outer-name
...
def apply_rotary_emb(x: torch.Tensor, freqs_cis: torch.Tensor) -> torch.Tensor:  # pylint: disable=redefined-outer-name
...
class MLA(nn.Module):
  ...
  def __init__(self, args: ModelArgs):  # pylint: disable=redefined-outer-name
    super().__init__()
    self.dim = args.dim
  ...
  def forward(
      self,
      x: torch.Tensor,  # pylint: disable=redefined-outer-name
      start_pos: int,
    ...
    """
    wkv_b = []  # This is unnecessary but worksaround a bug in pycheck.
    bsz, seqlen, _ = x.size()
...
class Gate(nn.Module):
  ...
  def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:  # pylint: disable=redefined-outer-name
...
class Block(nn.Module):
  ...
  def __init__(self, layer_id: int, args: ModelArgs):  # pylint: disable=redefined-outer-name
