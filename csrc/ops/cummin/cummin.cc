...
#include "csrc/ops/cummin/cummin.h"
...
absl::StatusOr<CumminOutputs> BuildCumminShlo(const int64_t dim,
  ...
  // Run the argmin in s32: the scan emitter's combiner only supports 4-byte
  // element types, and the kernel already caps the scan length at int32 max.
  // torch.cummin wants s64 indices, so the s32 index output is widened back to
  // s64 after the scan.
  const mlir::IntegerType i32 = builder.getOpBuilder().getI32Type();

  // The running argmin needs each element's position. An associative scan body
  ...
  // Cumulative min. On equal values keep the highest index, matching
  // torch.cummin. min with max-index-on-tie is associative (it selects the
  // (value, index) pair with the smallest value, breaking ties by largest
  // index), so the result is independent of the scan's tree reordering
  // (is_associative).
  MultiInputScanBodyBuilder body_builder =
    ...
    const mlir::Value new_val =
        mlir::stablehlo::MinOp::create(op_builder, loc, run_val, cur_val)
            .getResult();
    const mlir::Value lt = mlir::stablehlo::CompareOp::create(
    ...
    const mlir::Value tie_or_greater =
        mlir::stablehlo::SelectOp::create(op_builder, loc, eq, max_idx, cur_idx)
            .getResult();
    const mlir::Value new_idx =
        mlir::stablehlo::SelectOp::create(op_builder, loc, lt, run_idx,
                                          tie_or_greater)
            .getResult();
  ...
  // [carries..., outputs...]; the per-position outputs are the cummin results.
  TT_ASSIGN_OR_RETURN(
