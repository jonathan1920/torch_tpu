...
#include "csrc/common/testing/error_utils_test_helper.h"
...
TEST(NormalizeTorchTpuVirtualIncludesFilePath, StripsPrefix) {
  // "csrc/" relative paths.
  EXPECT_EQ(Normalize("csrc/foo.cc"), "csrc/foo.cc");
  EXPECT_EQ(Normalize("bazel-out/k8-fastbuild/bin/csrc/foo.cc"), "csrc/foo.cc");
  EXPECT_EQ(Normalize("bazel-out/k8-fastbuild/bin/csrc/ops/_virtual_includes/"
                      "op_builder_utils/csrc/ops/op_builder_utils.h"),
            "csrc/ops/op_builder_utils.h");
  ...
  EXPECT_EQ(
      ...
      "third_party/py/torch_tpu/foo.cc");
  EXPECT_EQ(
      Normalize("bazel-out/k8-fastbuild/bin/third_party/py/torch_tpu/csrc/ops/"
                "_virtual_includes/op_builder_utils/third_party/py/"
                "torch_tpu/csrc/ops/op_builder_utils.h"),
      "third_party/py/torch_tpu/csrc/ops/op_builder_utils.h");
}
...
TEST(TtError, HeaderPathNormalization) {
  ...
  constexpr std::string_view kExpectedHeaderPath =
      "third_party/py/torch_tpu/csrc/common/testing/error_utils_test_helper.h";
#else
  constexpr std::string_view kExpectedHeaderPath =
      "csrc/common/testing/error_utils_test_helper.h";
#endif  // TT_IS_INTERNAL_TORCH_TPU

  // Verify it contains the normalized path to the header.
  EXPECT_THAT(trace, testing::HasSubstr(kExpectedHeaderPath));
