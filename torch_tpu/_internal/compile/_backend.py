...
def _serialization_context(enable: bool = False):
  """Optionally configure aot_autograd to produce a BundledAOTAutogradResult.
  ...
  Args:
    enable: If True, activate the serialization patches and yield a
      one-element list whose ``[0]`` slot will hold the captured
      ``BundledAOTAutogradResult`` after compilation.  If False, yield
      None and do nothing.
  ...
  with ExitStack() as stack:
    ...
    orig_cache_inference_info = _graph_compile._cache_inference_info

    def _patched_cache_inference_info(
        aot_config, fw_metadata, maybe_subclass_meta,
        compiled_fw, aot_forward_graph_str, wrappers,
    ):
      ...
      _autograd_cache.AOTAutogradCache.generate_guards_expression = staticmethod(
          lambda *a, **kw: None
      )
      try:
        ...
        _autograd_cache.AOTAutogradCache.generate_guards_expression = orig_guards
    ...
    _graph_compile._cache_inference_info = _patched_cache_inference_info
    stack.callback(
        setattr, _graph_compile, "_cache_inference_info",
...
class TpuBackend:
  ...
  def __init__(
    ...
    Args:
      ...
      enable_serialization: If True, enable the aot_autograd bundled
        cache so that .serialize() is attached to compiled functions.
        This allows compilation artifacts to be saved/loaded without
        re-running aot_autograd.
    """
    self._debug = debug
  ...
  def __call__(
    ...
    with _serialization_context(self._enable_serialization) as captured_entry:
      bw = functools.partial(self._compile_graph_module) if self._enable_serialization else None
      result = aot_autograd(
