...
absl::StatusOr<DeviceBufferRef> Bmm(const at::Tensor& self,
  ...
  auto op_builder = [output_dtype_mlir](FixedSizeSpan<mlir::MlirOp, 2> inputs)
      -> absl::StatusOr<mlir::MlirOp> {
    auto& [self_op, mat2_op] = inputs;
    TT_ASSIGN_OR_RETURN(auto result,
                        BuildBmmShlo(self_op, mat2_op, output_dtype_mlir));
    return result;
  };
  TT_ASSIGN_OR_RETURN(
      auto result_buffer,
      DispatchOp<2>(
          OpName::kBmmDtypeOut, std::move(op_builder), {self, mat2},
          {.out_dtype = output_dtype_mlir, .out_dims = output_dims_vec}));
...
at::Tensor AtenBmmDtype(const at::Tensor& self, const at::Tensor& mat2,
                        at::ScalarType out_dtype) {
  TT_KERNEL(OpName::kBmm, _, (self, mat2, out_dtype), {
    TT_ASSIGN_OR_THROW(auto result_buffer, Bmm(self, mat2, out_dtype));
...
at::Tensor& AtenBmmDtypeOut(const at::Tensor& self, const at::Tensor& mat2,
  ...
  TT_KERNEL(OpName::kBmm, _, (self, mat2, out_dtype, out), {
