...
at::Tensor AtenUnfold(const at::Tensor& self, int64_t dimension, int64_t size,
                      int64_t step) {
  TT_KERNEL(OpName::kUnfold, _, (self, dimension, size, step), {
    ...
    TT_CHECK_THROW(dimension >= -dims && dimension < dims, error::kOutOfRange)
        << "expected dimension to be in range of [" << -dims << ", " << dims - 1
        << "] for shape " << self.sizes() << ", got " << dimension;
    ...
    const int64_t num_windows = (self.size(normalized_dim) - size) / step + 1;
