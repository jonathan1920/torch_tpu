...
void AtenSplitWithSizesCopyOut(const at::Tensor& self,
                               ...
                               at::TensorList out) {
  TT_KERNEL(OpName::kSplitWithSizesCopyOut, _, (self, split_sizes, dim, out), {
    // split_with_sizes_copy is an undocumented op used by PyTorch's
    // fully_shard() wrapper (FSDP2). The default decomposition, found in
    // https://github.com/pytorch/pytorch/blob/main/torch/_decomp/decompositions.py,
    // performs one copy per split. For FSDP2, the input is a 2-d coalesced
    // tensor of shape (fsdp_world_size, -1). The output slices are reshaped
    // into the unsharded parameters. The resulting multi-dimensional reshapes
    // are slow to compile and execute. This implementation does a more
    // performant sequence of slices and concatenations in the 2-d case.
    // The performance benefits might generalize to other dimensionalities,
    // but for simplicity we choose for now to only target the known use case.
    if (dim == 1 && self.dim() == 2) {
      const int64_t num_rows = self.size(0);
      const at::Tensor flat_self = self.view(-1);

      const int64_t num_params = out.size();
      std::vector<int64_t> all_split_sizes;  // INT_VEC_OK=Not tensor dimension
