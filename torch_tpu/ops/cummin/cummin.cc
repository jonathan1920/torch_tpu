...
absl::StatusOr<CumminOutputs> BuildCumminShlo(const int64_t dim,
  ...
  const auto body_builder = [carry_shape](
    ...
    const mlir::Value lt_pred = mlir::stablehlo::CompareOp::create(
                                    op_builder, loc, carry_val, input_val,
                                    mlir::stablehlo::ComparisonDirection::LT)
                                    .getResult();

    const mlir::Value selected_value =
        mlir::stablehlo::SelectOp::create(op_builder, loc, lt_pred, carry_val,
                                          input_val)
            .getResult();

    const mlir::Value carry_idx = carries[1];
    const mlir::Value argmin_index =
        mlir::stablehlo::SelectOp::create(op_builder, loc, lt_pred, carry_idx,
                                          input_idx)
            .getResult();

    return llvm::SmallVector<mlir::Value>{selected_value, argmin_index};
  };
  ...
  return CumminOutputs{.values = results[0], .indices = results[1]};
}
