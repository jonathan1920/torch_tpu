...
// Builds the cumulative product operation using StableHLO's ReduceWindowOp.
//
// How it works:
// The cumulative product along a dimension `dim` means that each element in the
// output is the product of all elements up to that index in the input tensor
// along `dim`.
//
// We can achieve this using `stablehlo.reduce_window` with `stablehlo.multiply`
...
//
// 2.  Padding: We add `N-1` padding elements (ones, the identity for
//     multiplication) to the *beginning* (low side) of the input tensor along
//     `dim`. No padding is added to the end (high side).
//
...
//     ones, each position of the window captures a prefix of the original
//     tensor.
//
// Example: Input `[a, b, c]`, dim=0. Size N=3.
// -   Padded input: `[1, 1, a, b, c]` (N-1 = 2 ones prepended).
// -   Window size: [3]. Stride: [1].
//
// -   Window 1: `[1, 1, a]` -> Reduce(Mul) = a
// -   Window 2: `[1, a, b]` -> Reduce(Mul) = a * b
// -   Window 3: `[a, b, c]` -> Reduce(Mul) = a * b * c
//
// The result is `[a, a*b, a*b*c]`, which is the cumulative product.
//
...
absl::StatusOr<mlir::MlirOp> BuildCumprodShlo(
  ...
  mlir::MlirOp init_value = MakeScalarConstant(builder, 1, element_type);
  TT_ASSIGN_OR_RETURN((auto [window_dimensions, window_strides, base_dilations,
                             window_dilations, padding]),
                      GetReduceWindowAttributes(builder, input_type, dim));

  return stablehlo::ReduceWindow(
      builder, /*inputs=*/{input}, /*init_values=*/{init_value},
      /*body=*/
      [&](mlir::RegionBuilder& body) {
        stablehlo::buildReduceBody<stablehlo::MulOp>(
