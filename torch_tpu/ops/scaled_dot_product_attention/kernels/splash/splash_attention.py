...
def flash_attention_kernel(
  ...
  def body(kv_compute_index, _, has_partial_mask=False):
    ...
    exp = jnp.exp2 if config.use_base2_exp else jnp.exp
    if max_logit_estimate is None:
      s_curr = exp(qk - jnp.tile(m_next, (1, bkv_repeats)))  # pyrefly: ignore[bad-argument-type, unsupported-operation]
    else:
    ...
    if max_logit_estimate is None:
      if isinstance(head_dim_v_repeats, int):
        alpha_o = jnp.tile(alpha, (1, head_dim_v_repeats))  # pyrefly: ignore[bad-argument-type]
      else:
        alpha_o = jnp.broadcast_to(alpha[:, 0:1], o_scratch_ref.shape)  # pyrefly: ignore[unsupported-operation]
  ...
  def end():
    ...
    if fuse_reciprocal:  # allows fusing reciprocal out of the kernel
      if isinstance(head_dim_v_repeats, int):
        ...
        l_inv = jnp.broadcast_to((1.0 / l)[:, 0:1], o_scratch_ref.shape)
