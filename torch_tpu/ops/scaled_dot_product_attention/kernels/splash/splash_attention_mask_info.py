...
def _process_mask(
  ...
  # During Pallas export with dynamic shapes, mask.shape contains _DimExpr
  # objects. We bypass offline mask traversal that requires concrete integer
  # shapes for allocation.
  is_dynamic = lambda shape: any(not isinstance(d, int) for d in shape)
  if is_dynamic(mask.shape) and hasattr(mask, 'mask_function'):

    q_sequence = jnp.arange(mask.shape[0], dtype=jnp.int32)
    return (
        MaskInfo(
            mask_next=None,
            active_rows=None,
  ...
  # TODO(b/447656860): checking the validity of the masks is slow for large masks.
