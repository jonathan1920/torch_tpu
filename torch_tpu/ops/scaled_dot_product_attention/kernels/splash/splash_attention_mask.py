...
# Inherit from _ComputableMask to support dynamic shapes during export,
# enabling the early return path in _process_mask.
class FullMask(_ComputableMask):
  ...
  def __init__(
    ...
    def full_mask_function(q_ids, kv_ids):
      # Combine Query [M, 1] and KV [1, N] vectors using bitwise OR (`|`).
      # This triggers broadcasting to generate the [M, N] output grid.
      # Since index headers are always non-negative (>= 0), it creates a matrix
      # filled entirely with `True` without reading static shapes offline.
      return q_ids | kv_ids >= 0
