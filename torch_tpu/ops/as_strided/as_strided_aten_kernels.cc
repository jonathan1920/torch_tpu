...
at::Tensor AtenAsStrided(const at::Tensor& self, c10::SymIntArrayRef size_sym,
  ...
  TT_KERNEL(OpName::kAsStrided, _, (self, new_sizes, new_strides, new_offset), {
    ...
    TT_ASSIGN_OR_THROW(mlir::ElementType tensor_element_type,
                       ConvertTo<mlir::ElementType>(self.scalar_type()));
    TT_THROW_IF_ERROR(ValidateStorageAndLayoutBytes(
        storage_numel, storage_element_type, new_sizes, new_strides, new_offset,
        tensor_element_type));

    // View is valid, so we can create the new tensor.
    // as_strided creates a shallow view; a new tensor with the same Storage
    // as the self tensor.
    ...
    at::Tensor tensor(c10::make_intrusive<c10::TensorImpl>(
        ...
        self.dtype()));
    tensor.unsafeGetTensorImpl()->set_sizes_and_strides(new_sizes, new_strides,
                                                        new_offset);
    return tensor;
  });
}
