...
#include "torch_tpu/csrc/common/testing/error_utils_test_helper.h"
...
TEST(NormalizeTorchTpuVirtualIncludesFilePath, StripsPrefix) {
  ...
  EXPECT_EQ(
      Normalize(
          "bazel-out/k8-fastbuild/bin/torch_tpu/csrc/ops/_virtual_includes/"
          "op_builder_utils/torch_tpu/csrc/ops/op_builder_utils.h"),
...
TEST(TtError, HeaderPathNormalization) {
  ...
  EXPECT_THAT(trace, testing::HasSubstr(absl::StrCat(
                         kProjectPrefix,
