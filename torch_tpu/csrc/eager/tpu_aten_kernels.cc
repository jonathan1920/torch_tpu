...
#include "torch_tpu/csrc/eager/tpu_aten_kernels.h"
...
#include "torch_tpu/csrc/ops/a_min_max/a_min_max_aten_kernels.h"
#include "torch_tpu/csrc/ops/addcdiv/addcdiv_aten_kernels.h"
#include "torch_tpu/csrc/ops/addcmul/addcmul_aten_kernels.h"
#include "torch_tpu/csrc/ops/addmm/addmm_aten_kernels.h"
#include "torch_tpu/csrc/ops/addmv/addmv_aten_kernels.h"
#include "torch_tpu/csrc/ops/all_any/all_any_aten_kernels.h"
#include "torch_tpu/csrc/ops/angle/angle_aten_kernels.h"
#include "torch_tpu/csrc/ops/arange/arange_aten_kernels.h"
#include "torch_tpu/csrc/ops/as_strided/as_strided_aten_kernels.h"
#include "torch_tpu/csrc/ops/assertion_aten_kernels.h"
#include "torch_tpu/csrc/ops/baddbmm/baddbmm_aten_kernels.h"
#include "torch_tpu/csrc/ops/bernoulli/bernoulli_aten_kernels.h"
#include "torch_tpu/csrc/ops/binary_aten_kernels.h"  // IWYU pragma: keep for AtenMulTensor, etc
#include "torch_tpu/csrc/ops/binary_cross_entropy/binary_cross_entropy_aten_kernels.h"
#include "torch_tpu/csrc/ops/bincount/bincount_aten_kernels.h"
#include "torch_tpu/csrc/ops/bmm/bmm_aten_kernels.h"
#include "torch_tpu/csrc/ops/bucketize/bucketize_aten_kernels.h"
#include "torch_tpu/csrc/ops/cat/cat_aten_kernels.h"
#include "torch_tpu/csrc/ops/channel_shuffle/channel_shuffle_aten_kernels.h"
#include "torch_tpu/csrc/ops/clamp/clamp_aten_kernels.h"
#include "torch_tpu/csrc/ops/col2im/col2im_aten_kernels.h"
#include "torch_tpu/csrc/ops/convolution/convolution_aten_kernels.h"
#include "torch_tpu/csrc/ops/copy_from/copy_from_aten_kernels.h"
#include "torch_tpu/csrc/ops/count_nonzero/count_nonzero_aten_kernels.h"
#include "torch_tpu/csrc/ops/ctc_loss/ctc_loss_aten_kernels.h"
#include "torch_tpu/csrc/ops/cummax/cummax_aten_kernels.h"
#include "torch_tpu/csrc/ops/cummin/cummin_aten_kernels.h"
#include "torch_tpu/csrc/ops/cumprod/cumprod_aten_kernels.h"
#include "torch_tpu/csrc/ops/cumsum/cumsum_aten_kernels.h"
#include "torch_tpu/csrc/ops/digamma/digamma_aten_kernels.h"
#include "torch_tpu/csrc/ops/distance/dist_aten_kernels.h"
#include "torch_tpu/csrc/ops/dot/dot_aten_kernels.h"
#include "torch_tpu/csrc/ops/dot/vdot_aten_kernels.h"
#include "torch_tpu/csrc/ops/dropout/dropout_aten_kernels.h"
#include "torch_tpu/csrc/ops/dynamic/dynamic_arange/dynamic_arange.h"
#include "torch_tpu/csrc/ops/dynamic/dynamic_broadcast/dynamic_broadcast.h"
#include "torch_tpu/csrc/ops/dynamic/dynamic_reshape/dynamic_reshape.h"
#include "torch_tpu/csrc/ops/dynamic/dynamic_slice/dynamic_slice.h"
#include "torch_tpu/csrc/ops/dynamic/set_dimension_logical_size/set_dimension_logical_size.h"
#include "torch_tpu/csrc/ops/elu/elu_aten_kernels.h"
#include "torch_tpu/csrc/ops/embedding/embedding_aten_kernels.h"
#include "torch_tpu/csrc/ops/equal/equal_aten_kernels.h"
#include "torch_tpu/csrc/ops/experimental/ragged_all_to_all/ragged_all_to_all_aten_kernels.h"
#include "torch_tpu/csrc/ops/experimental/ragged_dot/ragged_dot_aten_kernels.h"
#include "torch_tpu/csrc/ops/experimental/send_recv/send_recv_kernels.h"
#include "torch_tpu/csrc/ops/experimental/sparse_dense_matmul/sparse_dense_matmul_aten_kernels.h"
#include "torch_tpu/csrc/ops/experimental/sparse_dense_matmul/sparse_dense_matmul_grad_with_adagrad_aten_kernels.h"
#include "torch_tpu/csrc/ops/experimental/sparse_dense_matmul/sparse_dense_matmul_grad_with_adam_aten_kernels.h"
#include "torch_tpu/csrc/ops/experimental/sparse_dense_matmul/sparse_dense_matmul_grad_with_sgd_aten_kernels.h"
#include "torch_tpu/csrc/ops/experimental/sparse_gather/sparse_gather_aten_kernels.h"
#include "torch_tpu/csrc/ops/exponential/exponential_aten_kernels.h"
#include "torch_tpu/csrc/ops/eye/eye_aten_kernels.h"
#include "torch_tpu/csrc/ops/fake_quantize/fake_quantize_aten_kernels.h"
#include "torch_tpu/csrc/ops/fake_quantize/fused_moving_avg_obs_fq_helper_aten_kernels.h"
#include "torch_tpu/csrc/ops/fft/fft_aten_kernels.h"
#include "torch_tpu/csrc/ops/fill/fill_aten_kernels.h"
#include "torch_tpu/csrc/ops/flip/flip_aten_kernels.h"
#include "torch_tpu/csrc/ops/fmax/fmax_aten_kernels.h"
#include "torch_tpu/csrc/ops/fmin/fmin_aten_kernels.h"
#include "torch_tpu/csrc/ops/foreach_aten_kernels.h"
#include "torch_tpu/csrc/ops/fused_adagrad/fused_adagrad_aten_kernels.h"
#include "torch_tpu/csrc/ops/fused_adam/fused_adam_aten_kernels.h"
#include "torch_tpu/csrc/ops/fused_adamw/fused_adamw_aten_kernels.h"
#include "torch_tpu/csrc/ops/fused_sgd/fused_sgd_aten_kernels.h"
#include "torch_tpu/csrc/ops/gather/gather_aten_kernels.h"
...
#include "torch_tpu/csrc/ops/histc/histc_aten_kernels.h"
#include "torch_tpu/csrc/ops/im2col/im2col_aten_kernels.h"
#include "torch_tpu/csrc/ops/index/index_aten_kernels.h"
...
#include "torch_tpu/csrc/ops/index_fill/index_fill_aten_kernels.h"
#include "torch_tpu/csrc/ops/index_put/index_put_aten_kernels.h"
#include "torch_tpu/csrc/ops/index_reduce/index_reduce_aten_kernels.h"
#include "torch_tpu/csrc/ops/index_select/index_select_aten_kernels.h"
#include "torch_tpu/csrc/ops/is/is_aten_kernels.h"
#include "torch_tpu/csrc/ops/isin/isin_aten_kernels.h"
#include "torch_tpu/csrc/ops/jagged/jagged_aten_kernels.h"
#include "torch_tpu/csrc/ops/layer_norm/layer_norm_aten_kernels.h"
...
#include "torch_tpu/csrc/ops/linalg/vector_norm/aten_vector_norm_kernels.h"
#include "torch_tpu/csrc/ops/linspace/linspace_aten_kernels.h"
...
#include "torch_tpu/csrc/ops/min_max/min_max_aten_kernels.h"
#include "torch_tpu/csrc/ops/mm/mm_aten_kernels.h"
#include "torch_tpu/csrc/ops/mse_loss/mse_loss_aten_kernels.h"
#include "torch_tpu/csrc/ops/multinomial/multinomial_aten_kernels.h"
#include "torch_tpu/csrc/ops/nan_to_num/nan_to_num_aten_kernels.h"
#include "torch_tpu/csrc/ops/native_batch_norm/native_batch_norm_aten_kernels.h"
#include "torch_tpu/csrc/ops/native_multi_head_attention/native_multi_head_attention_aten_kernels.h"
...
#include "torch_tpu/csrc/ops/prelu/prelu_aten_kernels.h"
#include "torch_tpu/csrc/ops/prod/prod_aten_kernels.h"
#include "torch_tpu/csrc/ops/put/put_aten_kernels.h"
...
#include "torch_tpu/csrc/ops/repeat_interleave/repeat_interleave_aten_kernels.h"
#include "torch_tpu/csrc/ops/replication_pad/replication_pad_aten_kernels.h"
#include "torch_tpu/csrc/ops/resize/resize_aten_kernels.h"
#include "torch_tpu/csrc/ops/rms_norm/rms_norm_aten_kernels.h"
#include "torch_tpu/csrc/ops/roll/roll_aten_kernels.h"
#include "torch_tpu/csrc/ops/round/round_aten_kernels.h"
#include "torch_tpu/csrc/ops/scaled_dot_product_attention/scaled_dot_product_attention_aten_kernels.h"
#include "torch_tpu/csrc/ops/scaled_mm/scaled_mm_aten_kernels.h"
#include "torch_tpu/csrc/ops/scatter/scatter_aten_kernels.h"
#include "torch_tpu/csrc/ops/searchsorted_aten_kernels.h"
#include "torch_tpu/csrc/ops/segment_reduce/segment_reduce_aten_kernels.h"
#include "torch_tpu/csrc/ops/set/set_aten_kernels.h"
#include "torch_tpu/csrc/ops/sigmoid/sigmoid_aten_kernels.h"
#include "torch_tpu/csrc/ops/softmax/_masked_softmax_aten_kernels.h"
#include "torch_tpu/csrc/ops/softmax/softmax_aten_kernels.h"
#include "torch_tpu/csrc/ops/softplus/softplus_aten_kernels.h"
#include "torch_tpu/csrc/ops/sort/sort_aten_kernels.h"
#include "torch_tpu/csrc/ops/split_with_sizes_copy/split_with_sizes_copy_aten_kernels.h"
#include "torch_tpu/csrc/ops/take/take_aten_kernels.h"
#include "torch_tpu/csrc/ops/tanh/tanh_aten_kernels.h"
#include "torch_tpu/csrc/ops/thnn_fused_gru_cell/thnn_fused_gru_cell_aten_kernels.h"
#include "torch_tpu/csrc/ops/thnn_fused_lstm_cell/thnn_fused_lstm_cell_aten_kernels.h"
#include "torch_tpu/csrc/ops/threshold/threshold_aten_kernels.h"
#include "torch_tpu/csrc/ops/to_copy/to_copy_aten_kernels.h"
#include "torch_tpu/csrc/ops/topk/topk_aten_kernels.h"
#include "torch_tpu/csrc/ops/transformer_encoder_layer_fwd/_transformer_encoder_layer_fwd_aten_kernels.h"
#include "torch_tpu/csrc/ops/triangular/triangular_aten_kernels.h"
#include "torch_tpu/csrc/ops/tril_indices/tril_indices_aten_kernels.h"
#include "torch_tpu/csrc/ops/unary_aten_kernels.h"
#include "torch_tpu/csrc/ops/unfold/unfold_aten_kernels.h"
#include "torch_tpu/csrc/ops/uniform/uniform_aten_kernels.h"
#include "torch_tpu/csrc/ops/unique/unique_aten_kernels.h"
#include "torch_tpu/csrc/ops/unique_consecutive/unique_consecutive_aten_kernels.h"
#include "torch_tpu/csrc/ops/upsample/upsample_aten_kernels.h"
#include "torch_tpu/csrc/ops/upsample/upsample_bicubic2d_aten_kernels.h"
#include "torch_tpu/csrc/ops/view/view_aten_kernels.h"
#include "torch_tpu/csrc/ops/weight_norm/weight_norm_aten_kernels.h"
#include "torch_tpu/csrc/ops/where/where_aten_kernels.h"
#include "torch_tpu/csrc/ops/xlogy/xlogy_aten_kernels.h"
...
TORCH_LIBRARY_IMPL(aten, PrivateUse1, m) {
  ...
  ImplStable<OpName::kScaledMmV2Out>(m, AtenScaledMmV2Out);
